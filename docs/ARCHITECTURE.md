...
- 呼び出し側が`boxes`を与えたときは、与えた座標がそのまま結果に戻り、与えた順が読み順になる。画像と
  ...
  ときだけ画像内に切り詰める。
- 行cropは上・下・右に`margin`（既定45px）の余白を付け、左には付けない。縦書きでは左隣が次の行だからである。原実装の`useOCRWorker.ts`は、
  学習データも「左=bbox端、上下右=+45px」で作られていると注記している。画像外は白。
...
- encoderはfp16配信。CPUでは`encoder_path`が同じ重みのfp32版を初回に作って隣にキャッシュし、来歴
  ...
  変換手順を変えたら`FP32_CONVERTER`を上げる。CUDAでは既定でfp16のまま使い、`precision="fp32"`
  （CLIでは`--encoder-precision fp32`）で変換版を強制できる。
...
出力の順は逐次処理と同じで、内容は同じページを10回処理した測定で一致した（あらゆる装置での同一性を
保証するものではない）。段階ごとの秒数の合計はページの経過時間を上回りうる。`cancelled`
