...
CPUではスレッド数を明示した方が速い。8コア16スレッドのこの機械では`--threads 8 --decoder-threads 2`（物理コア数と2）が
3コマの測定で既定（onnxruntime任せ）より2〜6割短く、実行ごとのばらつきも小さかった（[benchmarks/README.md](benchmarks/README.md)）。
既定値は変えていない。
...
ある。計80コマで、文字の取りこぼしは正規化後6〜7%、読み順と行の切り方まで含めた文字誤り率は14〜16%だった。読み順とKoji変換の原実装との比較は[benchmarks/README.md](benchmarks/README.md)を参照。
